...
function setupStudioEditor(
  ...
  function updateGroupHistoryButtons(): void { groupPanel.querySelector<HTMLButtonElement>("#studioGroupUndo")!.disabled = groupUndo.length === 0; groupPanel.querySelector<HTMLButtonElement>("#studioGroupRedo")!.disabled = groupRedo.length === 0; }
  ...
  groupPanel.querySelector<HTMLButtonElement>("#studioGroupUndo")!.addEventListener("click", () => { const previous = groupUndo.pop(); if (!previous) return; groupRedo.push(snapshotGroups()); restoreGroupSnapshot(previous); });
  groupPanel.querySelector<HTMLButtonElement>("#studioGroupRedo")!.addEventListener("click", () => { const next = groupRedo.pop(); if (!next) return; groupUndo.push(snapshotGroups()); restoreGroupSnapshot(next); });
