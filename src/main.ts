...
function renderPhotoGrid(): void {
  ...
  if (photos.length === 0) { grid.innerHTML = `<div class="empty-state"><strong>No photos loaded</strong><span>Choose an input folder to build the thumbnail workspace.</span></div>`; renderInspector(); updatePhotoStats(); window.dispatchEvent(new CustomEvent("tihulu:workspace-grid-rendered")); return; }
  grid.innerHTML = "";
